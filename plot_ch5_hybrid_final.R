...
main_file <- "results_main_with_hybrid_final.csv"
if (!file.exists(main_file)) stop("Missing ", main_file)
...
main_df <- main_df %>%
  ...
  mutate(
    ...
    B_tune_sec_num = as.numeric(B_tune_sec),
...
main_sum <- main_df %>%
  ...
  summarise(
    n_seed = sum(is.finite(valid_power)),
  ...
  filter(n_seed > 0, is.finite(mean_valid_power))
...
p_main <- ggplot(
  ...
  geom_errorbar(
    aes(ymin = pmax(mean_valid_power - se_valid_power, 0),
        ymax = pmin(mean_valid_power + se_valid_power, 1)),
    width = 5, linewidth = 0.5, na.rm = TRUE
  ) +
...
ggsave(file.path(fig_dir, "ch5_main_mean_valid_power_with_hybrid.png"), p_main,
...
p_rank <- ggplot(rank_df, aes(x = B_tune_sec, y = method, fill = rank_valid_power)) +
  ...
  facet_wrap(~ theta_set_size, nrow = 2) +
  scale_fill_gradient(low = "grey20", high = "grey90",
                      breaks = 1:length(main_methods),
                      name = "Rank\n(1=best)") +
  labs(x = expression(B[tune] ~ "(seconds)"), y = NULL) +
  theme_main

ggsave(file.path(fig_dir, "ch5_main_rank_heatmap_with_hybrid.png"), p_rank,
       width = 10, height = 7, dpi = 300)

# Ablation plot.
abl_file <- "results_ch5_hybrid_ablation_final.csv"
if (file.exists(abl_file)) {
  ...
  abl <- abl %>%
    ...
    mutate(
      ...
      B_tune_sec = factor(B_tune_sec, levels = c(80, 160, 320))
    )
  ...
  p_abl <- ggplot(
    abl_sum,
    aes(x = B_tune_sec, y = mean_valid_power,
  ...
  ggsave(file.path(fig_dir, "ch5_hybrid_ablation_valid_power.png"), p_abl,
         width = 11, height = 6, dpi = 300)
}

# Lambda sensitivity plot.
lam_file <- "results_ch5_lambda_sensitivity_final.csv"
if (file.exists(lam_file)) {
  ...
  ggsave(file.path(fig_dir, "ch5_lambda_sensitivity.png"), p_lam,
         width = 10, height = 6, dpi = 300)
}

message("Chapter 5 final figures written to ./", fig_dir, "/")
print(list.files(fig_dir, pattern = "^ch5_.*\\.png$", full.names = TRUE))
